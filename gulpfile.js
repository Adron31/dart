...
let path = {
   ...
   build: {
      ...
      sprite: projectFolder + "/img/icons/sprite/",
   ...
   src: {
      ...
      img: {
         ...
         icons: [
            sourceFolder + "/img/icons/**/*.{jpg,png,svg,gif,ico,webp}",
            "!" + sourceFolder + "/img/sprite/*.{png,jpeg,svg}",
            sourceFolder + "/img/sprite/sprite.{jpg,png,svg}",
         ]
      },
      sprite: sourceFolder + "/img/icons/sprite/",
...
function fonts () {                               // преобразование ttf в woff and woff2
   src([sourceFolder + "/fonts/fontsicon/*"])
      .pipe(dest(projectFolder + "/fonts/fontsicon/"));
...
let build = series(clean, parallel(js, mincss, css, html, images, fonts, others));
